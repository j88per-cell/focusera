  ...
  <AdminLayout>
    <div class="max-w-5xl mx-auto p-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <a href="/admin" class="text-sm text-accent hover:underline">Back to Admin</a>
      </div>

      <div v-if="!settings.length" class="text-sm text-gray-600 bg-white rounded border p-4">
        No settings found yet. When settings are created, they will appear grouped here.
      </div>

      <div v-else class="space-y-3">
        <div v-for="group in grouped" :key="group.name" class="bg-white rounded-md border">
          <button type="button" class="w-full flex items-center justify-between px-4 py-3 hover:bg-gray-50"
                  @click="toggleGroup(group.name)">
            <div class="font-medium text-gray-800">{{ group.name }}</div>
            <div class="text-xs text-gray-500">{{ group.subgroups.length }} subgroup(s)
              <span class="ml-2" aria-hidden="true">{{ openGroups[group.name] ? '−' : '+' }}</span>
            </div>
          </button>
          <div v-show="openGroups[group.name]" class="border-t">
            <div v-for="sub in group.subgroups" :key="group.name + '::' + sub.name" class="border-b last:border-b-0">
              <button type="button" class="w-full flex items-center justify-between px-6 py-2 hover:bg-gray-50"
                      @click="toggleSub(group.name, sub.name)">
                <div class="text-sm text-gray-700">{{ sub.name }}</div>
                <div class="text-xs text-gray-500">{{ sub.items.length }} setting(s)
                  <span class="ml-2" aria-hidden="true">{{ openSubs[group.name + '::' + sub.name] ? '−' : '+' }}</span>
                </div>
              </button>
              <div v-show="openSubs[group.name + '::' + sub.name]" class="px-6 py-3">
                <div class="overflow-x-auto">
                  <table class="min-w-full text-sm">
                    ...
                    <tbody>
                      <tr v-for="item in sub.items" :key="item.id" class="border-b last:border-b-0">
